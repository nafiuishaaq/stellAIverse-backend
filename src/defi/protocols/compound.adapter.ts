...
export class CompoundAdapter implements ProtocolAdapter {
  ...
  async getPosition(address: string, token: string, chain: string = 'ethereum'): Promise<PositionData> {
    try {
      ...
      const comptroller = new ethers.Contract(this.comptrollerAddress, COMPOUND_COMPTROLLER_ABI, provider);
      ...
      const valueUSD = Number(ethers.formatUnits(underlyingBalance, 18)) * price;
  ...
  async getAllPositions(address: string, chain: string = 'ethereum'): Promise<PositionData[]> {
    try {
      ...
      const positions = await Promise.all(tokens.map((t) => this.getPosition(address, t, chain)));
  ...
  async withdraw(address: string, token: string, amount: number, chain: string = 'ethereum'): Promise<TransactionData> {
    ...
    const cTokenAmount = (Number(ethers.parseEther(amount.toString())) * 1e18) / (await this.getExchangeRate(token));
  ...
  async estimateGas(tx: TransactionData): Promise<GasEstimate> {
    ...
    return {
      ...
      costUSD: Number(ethers.formatEther(gasEstimate * BigInt('50000000000'))) * 2500,
    };
  }
