...
export class ReferralNotificationService {
  ...
  async sendReferralInvite(dto: SendReferralInviteDto): Promise<Referral> {
    ...
    this.logger.log(
      `Referral invitation sent to ${dto.refereeEmail} from user ${dto.referrerId}`,
    );
  ...
  async notifyRegistration(
    ...
    await this.logReferralEvent(
      referral.id,
      ReferralEventType.REGISTRATION_COMPLETED,
      {
    ...
    this.logger.log(
      `Referral registration notified for ${referral.refereeEmail}`,
    );
  }
  ...
  async notifyRewardEarned(
    ...
    await this.logReferralEvent(
      referral.id,
      ReferralEventType.REWARD_EARNED,
      rewardDetails,
    );
  ...
  async notifyMilestone(
    ...
    await this.logReferralEvent(
      referral.id,
      ReferralEventType.MILESTONE_REACHED,
      milestone,
    );
  ...
  private async sendReferralInviteEmail(data: {
    ...
    try {
      ...
      this.logger.error(
        `Failed to send referral email: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async getReferralStats(userId: string): Promise<{
    ...
    const stats = {
      ...
      registeredReferrals: referrals.filter(
        (r) => r.status === ReferralStatus.REGISTERED,
      ).length,
